...
var enemy_scenes = [
	"res://Game/Enemies/EnemyShotgun.tscn",
	"res://Game/Enemies/CommonShooter.tscn",
	"res://Game/Enemies/DetectDenShoot.tscn",
	"res://Game/Enemies/EnemyBurst.tscn"
]
...
func _ready():
	...
	respawn_timer_left.wait_time = 3.0
	...
	respawn_timer_right.wait_time = 3.0
...
func spawn_top_shooter():
	if current_top_shooters < max_shooters:
		top_shooter = load(enemy_scenes[randi() % enemy_scenes.size()]).instantiate()
		if randf() > 0.5:
...
func spawn_bottom_shooter():
	if current_bottom_shooters < max_shooters:
		bottom_shooter = load(enemy_scenes[randi() % enemy_scenes.size()]).instantiate()
...
func spawn_left_shooter():
	if current_left_shooters < max_shooters:
		left_shooter = load(enemy_scenes[randi() % enemy_scenes.size()]).instantiate()
		left_shooter.custom_velocity.x = left_shooter.SPEED
...
func spawn_right_shooter():
	if current_right_shooters < max_shooters:
		right_shooter = load(enemy_scenes[randi() % enemy_scenes.size()]).instantiate()
		right_shooter.custom_velocity.x = -right_shooter.SPEED
